...
const struct cam_mipi_sensor_mode *cam_mipi_imx471_mode(u8 mode)
{
	const struct cam_mipi_sensor_mode * const modes[] = {
		sensor_front_top_mipi_A_mode,
		sensor_front_top_mipi_B_mode,
		sensor_front_top_mipi_C_mode,
		sensor_front_top_mipi_D_mode,
	};

	if (mode > num_front_top_mipi_setting || mode >= ARRAY_SIZE(modes))
		mode = 0;
	return modes[mode];
}
